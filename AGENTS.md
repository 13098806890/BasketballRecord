...
- **存储相关的改动必须考虑向后兼容**（如 UserDefaults key 变更、JSON 序列化字段增减、Core Data 模型变更等），确保旧版本存储的数据不会被破坏
...
3. **归属确认**：每次更新 release note 前，先确认当前开发周期对应的版本号（`RELEASE_NOTES.md` 顶部第一个非空版本 section），所有新增改动只放入该版本 section，不得放入历史版本。
...
## 对话与提交规范
1. **开始写代码前，先输出「俺要开始咯」。**
2. **我没有让你 commit 之前，不要执行任何 `git commit`。**
3. **如果有想不清楚、疑惑、需要选择的地方，停下来思考，然后问我确认。**
