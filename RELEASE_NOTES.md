...
- **记分页面 B 方案 UI**：重构比赛记分页的球队并排布局、双色记分牌、紧凑球员卡片、四列操作按钮、事件卡片与透明语音输入入口；保留原有记分、撤销、换人、迟到球员、节次控制和比赛数据入口。
- **记分页设备适配**：针对高屏 iPhone 增加记分牌与球员区域的可用高度，支持 3/4/5 名上场球员的自适应排列，并兼容三位数比分。
- **比赛视觉统一**：记分页使用全局篮球背景，主队与客队采用不同记分牌颜色，命中/未中/犯规/暂停等操作按钮按功能语义区分颜色。
- **多语言产品页更新**：更新 App Store 截图和产品介绍，分别突出实时记分、球员统计、AI 赛后复盘、球队协同与生涯数据。
- **商店文案优化**：整理各语言描述与关键词，提升用户理解核心功能的效率。
- **设置页面 UI**：按统一篮球编辑风格重构设置主页，保留语音、iCloud、单位、Pro、AI、同步导入、隐私帮助及开发者入口，并新增语言说明入口。

### 修复

- **比赛详情统计展示**：恢复球队球员完整列表和完整统计摘要的默认展示，避免新版界面需要额外操作才能查看原有数据。
- **比赛详情入口边界**：进行中的比赛不再显示历史事件编辑入口；历史事件编辑、统计重算和修改时间保持一致。
- **比赛数据同步时间**：统一自动保存、事件编辑、分组和球员/球队合并的修改时间，保留比赛创建时间，避免云端冲突判断失真。
- **球队图标合并**：球队合并时保留目标队图标或继承源队图标，并清理已删除球队的孤立图标文件。
- **云端开关兼容性**：iCloud KVS 不可用时回退本地存储，避免 Simulator 或缺少 entitlement 时丢失云端比赛开关状态。
- **设置页底部安全区**：修复底部入口可能被 TabBar 或安全区遮挡的问题。

## 2.39 (2026-08-21)

### 新增

- **电竞像素皮肤**：设置中新增经典皮肤与电竞像素皮肤切换，像素皮肤暂不区分 Pro 与非 Pro。
- **球员位置**：球员数据结构、编辑球员页面及球员详情页支持记录和展示球员位置。
- **像素默认头像**：为深色像素皮肤增加可识别的默认球员头像。

### 改进

- **页面皮肤隔离**：比赛记录、生涯和球员详情的像素视觉拆分到独立 SwiftUI 文件，经典皮肤继续使用原有布局和逻辑。
- **球员详情数据排版**：整合出场、首发、替补、胜率、PPS、eFG 和 TS，生涯与场均数据并排展示，时间显示为分钟和秒。
- **像素统计展示**：统一像素风格的颜色、紧凑间距、数据分组、导航栏和底部 Tab 样式。
- **设置与勋章调整**：隐藏设置页的皮肤和勋章入口，暂停勋章发放及球员详情展示，保留已有勋章记录。
- **AI 总结多语言输出**：为全部支持语言增加明确的输出语言约束，球队统计标题和日期格式跟随当前语言与地区，并统一逐节总结的篇幅要求。
- **Excel 导出流程**：比赛记录支持选择多场比赛，并将每场比赛分别导出为 Excel 后打包成 ZIP；通过系统分享交付文件，生涯与球员等入口可选择对应比赛范围。
- **导出状态与本地化**：比赛加载和文件生成期间显示等待状态；Excel 表头、工作表名称和事件描述跟随当前语言。

### 修复

- **AI 总结本地化**：补齐韩语界面及多语言提示词漏译，移除硬编码英文和跨语言中文分区引用，避免总结出现英文或混合语言。
- **Excel 导出测试**：修正球员场均数据测试读取错误列的问题，恢复完整测试套件通过。
- **比赛事件导出**：修正事件时间和比赛暂停/继续等事件的导出内容，并保留助攻、得分及抢断相关球员信息。
- **事件流编辑与统计一致性**：恢复比赛记录中的编辑入口；修正事件修改、删除与恢复后的日志状态和统计重算，并保留编辑前的事件时间、节次及计时信息。
- **多语言文案**：复核并修复多个页面及 Excel 导出中的漏译和繁体中文用字问题。

## 2.38 (2026-08-17)

### 改进

- **正负值统一引擎**：实时记录、历史分析与日志编辑重算共用同一套正负值计算逻辑；球队模式下不再给该队球员记录正负值，对手球队（球员模式）的正负值保持正确。
- **比赛列表比分**：球员生涯选场次、比赛分组、蓝牙同步等列表在球队模式下正确显示球队比分，不再显示 0。
- **MVP 奖章识别**：支持结构化「MVP：球员姓名」输出与多语言关键词，提升 AI 总结中 MVP 奖章识别的准确率。
- **iCloud 同步冲突处理**：上传前比较云端更新时间，避免用旧数据覆盖云端新数据；云端较新的比赛会同步覆盖本地。
- **球队模式说明提示**：新建比赛开启「按球队统计」开关时显示说明，提示该模式整队统一记录、无需选择首发。
- **评分邀请**：完成第 3 场和第 12 场比赛后，邀请用户为 App 评分。

### 修复

- **恢复未完成比赛**：重新打开进行中的比赛后保留撤销历史，可继续撤销之前的所有操作。
- **俄语语音识别**：补全 `тли`（三分）、`дла`（两分）、`вал`（犯规）等常见口语音变词及「2 очка」教程说法。
- **云端数据保存**：数据保存改为按需写入，仅更新变化的比赛记录，减少不必要的磁盘写入。
- **本地化补全**：补充照片上传、删除、API 密钥、数据空态等 7 个缺失的多语言文案，并清理 3 个失效条目。
...
- **勋章显示开关**：设置页新增「显示勋章」开关，可隐藏球员详情页的勋章区域。

### 改进

- **批量换人**：换人页面改为直接展示场上/场下球员，支持一次性调整多名场上球员，系统自动计算换人对次。
- **SCF 分享服务**：分享数据改为按月目录存储，便于手动管理。
- **AI 总结 MVP 格式**：提示词要求 AI 以 `MVP：球员姓名` 格式输出 MVP，提升识别准确率；解析器优先读取结构化格式，读取不到时回退到正则提取。

### 修复

- **API Key 删除修复**：删除 API Key 时同时清理旧版遗留 key，防止重新出现。
- **球员编辑保存**：编辑球员时不再丢失球员分组和勋章信息；编辑保存后球员列表立即刷新，不再需要切换页面才能看到更新。
- **语音识别事件显示**：语音输入的原始文本不再直接写入比赛事件流，恢复显示标准化的「球员名 + 动作」格式。
- **AI 总结 MVP 头像**：修复 AI 总结的 MVP 头像因全文子串匹配显示错误的问题，改为结构化提取 + 逐项匹配。
...
- **事件流自动滚动**：新事件产生时事件流自动滚动到顶部显示最新事件。
- **语音成功动画**：语音输入成功后，屏幕上方显示球员头像和动作名称动画，停留 1.2 秒后淡出。
- **语音成功动画开关**：语音设置页新增「显示成功动画」开关，可关闭动画。
- **球员昵称**：球员编辑页面支持添加多个昵称，语音输入时说昵称即可匹配到球员。
- **球员搜索**：球员管理页面顶部新增搜索栏，可按姓名、昵称、球衣号码搜索球员。
- **每节正负值修复**：修复每节开始重置场上阵容的问题，换人状态自然延续到下一节；全场正负值统一使用逐节求和，消除不一致。
- **originalPeriodCount 初始化修复**：新建比赛时正确传入配置节数，多节比赛不再错误标记为加时；蓝牙/云端传输新增该字段。
- **AI 总结消失修复**：AISummaryView 从 store 读取最新 aiSummary，解决因值类型副本过期导致总结不显示的问题。
- **云端 AI 总结同步**：CloudKit 同步时检查本地缺少 AI 总结的比赛，从云端补充；上传包含 AI 总结。
- **导入保护**：`upsertSavedGames` 在入参无 AI 总结时保留本地已有的总结。
- **中国区合规**：`Locale.current.region == "CN"` 时仅显示 DeepSeek 一个 AI provider；元数据 description 移除 OpenAI/Anthropic 引用。
...
- **语音识别原文保留**：语音输入的原始文本（如"张三快攻命中"）直接写入日志，不再显示为"2PT Made"。
- **快攻得分统计**：新增 `fastBreakPoints` 字段，语音含"快攻"时自动累加快攻得分。
- **数据范围支持正负值**：选择特定节次时，正负值按该节的场上阵容重新计算。
- **中文语音新指令**：新增投篮、快攻、突破、跳投四个投篮指令。
- **中文语音教程**：添加投篮、快攻、突破、跳投共 8 个教程任务（简繁各 8 个）。
- **SCF 云存储 manifest 清理**：上传时维护 manifest 文件，支持按 TTL 清理过期共享。
- **新建比赛布局调整**：比赛设定选项移至表单最上方，优先展示。
